...
void setup() {
  ...
  // ota.setOtaPort(8266);  // Port defaults to 8266
  // ota.setOtaPassword((const char *)"123"); // No authentication by default

  ota.ledON(LED_BUILTIN);  // flash the onboard LED during Wi-Fi connection

  // SSID, PASSWORD and HOSTNAME for mDNS & OTA use
  if(ota.beginWiFiOTA(STA_SSID, STA_PASS,HOSTNAME) < 0) {
